...
class OverlayMenu
{
	...
	void display()
	{
		...
		case cOverlayMainMenu:
			...
			{
				text("Output Serial Port: "+Serial.list()[matrix.serialPortNum], xOffset+20, yOffset+110);
				text("Output Serial Baud: "+matrix.serialBaud, xOffset+20, yOffset+130);
			} 
			else if(matrix.transmissionType == 3)
			{
				text("Artnet Enabled, transmitting "+ceil((float)TransmissionArray.length/512)+" universes", xOffset+20, yOffset+110);	
				text("Output IP Adr: "+matrix.outputNetworkIPAdr, xOffset+20, yOffset+130);
			} 
			
			text("Output Target FPS: "+matrix.outputFPS+"mS", xOffset+20, yOffset+150);
